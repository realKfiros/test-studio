...
const state = { catalog: null, runner: "all", workspace: "all", query: "", platform: "all", file: null, tab: "tests", selected: new Map(), runs: [], runId: null, run: null, jobId: null, showHistory: false };
const supported = file => ["bun", "maestro"].includes(file.runner);
const token = $('meta[name="test-studio-token"]').content;
let polling = false;
let sourceRequest = 0;
const duration = ms => ms < 1000 ? `${Math.round(ms)}ms` : ms < 60000 ? `${(ms / 1000).toFixed(1)}s` : `${Math.floor(ms / 60000)}m ${Math.round(ms % 60000 / 1000)}s`;
async function api(path, body) {
    const response = await fetch(path, { method: body === undefined ? "GET" : "POST", headers: { "x-test-studio-token": token, ...(body === undefined ? {} : { "Content-Type": "application/json" }) }, ...(body === undefined ? {} : { body: JSON.stringify(body) }) });
...
function filtered() {
    if (!state.catalog)
        return [];
    return state.catalog.files.filter(file => (state.runner === "all" || file.runner === state.runner) && (state.workspace === "all" || file.workspace === state.workspace) && (state.platform === "all" || file.platform === state.platform) && (!state.query || [file.path, file.name, ...file.cases.map(test => test.fullName), ...(file.tags ?? [])].join(" ").toLowerCase().includes(state.query)));
}
function summary() {
    const c = state.catalog;
    $("#project-name").textContent = c.name;
    $("#project-name").title = c.root;
    const stats = [["▤", c.files.length, "test files", "Auto-discovered"], ["⌘", c.files.reduce((n, f) => n + f.cases.length, 0), "test declarations", "From source"], ["◈", c.files.filter(f => f.runner === "maestro").length, "Maestro flows", "iOS + Android"]];
    $("#stats").innerHTML = stats.map(([icon, count, label, note]) => `<div class="stat"><span class="stat-icon">${icon}</span><div><div class="stat-value">${count.toLocaleString()}</div><div class="stat-label">${label}</div></div><span class="stat-note">${note}</span></div>`).join("");
    $("#libraries").textContent = `Detected: ${c.libraries.join(" · ") || "none yet"}`;
    $("#scan-time").textContent = `Scanned ${new Date(c.scannedAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit", second: "2-digit" })}`;
    $("#runners").innerHTML = [["all", "▦", "All tests"], ["bun", "◉", "Bun tests"], ["maestro", "◈", "Maestro flows"]].map(([id, icon, label]) => `<button data-runner="${id}" aria-label="${label}" title="${label}" class="${state.runner === id ? "active" : ""}"><span class="nav-icon">${icon}</span><span class="nav-text">${label}</span><span class="count">${c.files.filter(f => id === "all" || f.runner === id).length}</span></button>`).join("");
    const workspaces = [...new Set(c.files.map(f => f.workspace))].sort();
    if (document.activeElement !== $("#workspace-picker")) {
        $("#workspace-picker").innerHTML = '<option value="all">All workspaces</option>' + workspaces.map(workspace => `<option value="${esc(workspace)}">${esc(workspace === "." ? "Project root" : workspace)}</option>`).join("");
        $("#workspace-picker").value = state.workspace;
    }
    $("#workspaces").innerHTML = workspaces.map(workspace => `<button class="workspace-item ${state.workspace === workspace ? "active" : ""}" data-workspace="${esc(workspace)}" title="${esc(workspace)}"><span class="nav-icon">⌑</span><span class="nav-text">${esc(workspace === "." ? "Project root" : workspace)}</span><span class="count">${c.files.filter(f => f.workspace === workspace).length}</span></button>`).join("");
    $("#breadcrumb").textContent = state.workspace !== "all" ? state.workspace : state.runner === "all" ? "All tests" : state.runner === "bun" ? "Bun tests" : "Maestro flows";
}
function renderFiles() {
    ...
    $("#files").innerHTML = files.map(file => {
        const heading = workspace !== file.workspace ? `<div class="workspace-heading"><span>⌄</span>${esc(file.workspace === "." ? "Project root" : file.workspace)}<span>${files.filter(f => f.workspace === file.workspace).length}</span></div>` : "";
        workspace = file.workspace;
        const count = file.runner === "maestro" ? `${file.steps?.length ?? 0}` : file.cases.length;
    ...
    document.querySelectorAll("[data-file-check]").forEach(input => { input.indeterminate = state.selected.get(input.dataset.fileCheck) instanceof Set; });
...
function selectionCount() {
    ...
    $("#run-selected").title = `${state.selected.size} files selected; individual test filters are preserved`;
}
function currentFile() { return state.catalog?.files.find(file => file.id === state.file?.id); }
function renderInspector() {
    ...
    if (file.runner === "maestro") {
        ...
        content += file.steps.map((step, index) => `<div class="test-row"><span class="step-number">${String(index + 1).padStart(2, "0")}</span><span class="test-name">${esc(step.name)}</span><span class="test-line">:${step.line}</span></div>`).join("");
    }
    else {
        let suite = null;
        content += file.cases.map(test => {
            const group = test.fullName.slice(0, -test.name.length).trim() || "File-level tests";
            const heading = suite !== group ? `<div class="suite-name"><span>⌄</span> ${esc(group)}</div>` : "";
            suite = group;
            const selected = state.selected.has(file.id) && (state.selected.get(file.id) === null || state.selected.get(file.id)?.has(test.id));
        ...
        if (!file.cases.length)
            content += '<p class="note">No static test declarations found. A file-level run discovers tests generated at runtime.</p>';
    }
...
async function renderSource(file) {
    ...
    try {
        ...
        if (request !== sourceRequest || state.runId || state.tab !== "source" || state.file?.id !== file.id)
            return;
        $("#file-content").innerHTML = `<pre class="source">${result.source.split("\n").map((line, index) => `<span class="source-line"><span class="source-number">${index + 1}</span>${esc(line)}</span>`).join("")}</pre>`;
    }
    catch (error) {
        showError(error);
    }
}
function selectFile(id) { state.file = state.catalog.files.find(file => file.id === id); state.runId = null; state.run = null; state.showHistory = false; renderFiles(); renderInspector(); renderHistory(); }
function renderRunHistory() {
    $("#inspector").innerHTML = `<div class="inspector-heading"><button class="back-button" id="back-to-file">← Back to explorer</button><div class="inspector-title"><h2>Recent runs</h2></div><p class="inspector-path">The last 20 runs in this server session.</p></div><div class="inspector-content">${state.runs.map(run => `<button class="job-button history-entry" data-run="${run.id}"><span class="badge ${run.status}">${run.status}</span><span class="job-name">${run.jobs.length === 1 ? esc(run.jobs[0].file.name) : `${run.jobs.length} files`}</span><span>${new Date(run.startedAt).toLocaleTimeString()}</span></button>`).join("") || '<p class="note">No runs yet. Select a file or test to get started.</p>'}</div>`;
}
function renderHistory() {
    $("#history-count").textContent = state.runs.length;
    $("#history").innerHTML = state.runs.map(run => `<button data-run="${run.id}" class="${run.id === state.runId ? "active" : ""}"><span class="history-status ${run.status}"></span><span class="nav-text">${run.jobs.length === 1 ? esc(run.jobs[0].file.name) : `${run.jobs.length} files`}</span><span class="count">${new Date(run.startedAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</span></button>`).join("") || '<p class="sidebar-empty">Your runs will appear here.</p>';
}
function renderRun() {
    const run = state.run;
    ...
    const job = run.jobs.find(job => job.id === state.jobId) ?? run.jobs.find(job => job.status === "running") ?? run.jobs[0];
    if (!job)
        return;
...
function applyCatalog(catalog) {
    const changed = !state.catalog || JSON.stringify(catalog.files) !== JSON.stringify(state.catalog.files) || JSON.stringify(catalog.runners) !== JSON.stringify(state.catalog.runners);
...
document.addEventListener("click", async (event) => {
    ...
    try {
        if (button.dataset.runner) {
            ...
            state.workspace = state.workspace === button.dataset.workspace ? "all" : button.dataset.workspace;
            summary();
            renderFiles();
        }
        ...
        else if (button.dataset.runFile)
            await start([{ fileId: button.dataset.runFile }]);
            ...
            await start([...state.selected].map(([fileId, cases]) => ({ fileId, ...(cases ? { caseIds: [...cases] } : {}) })));
        else if (button.id === "select-visible") {
            for (const file of filtered())
                if (supported(file))
                    state.selected.set(file.id, null);
            renderFiles();
            if (!state.runId)
                renderInspector();
        }
            ...
            await start(state.run.jobs.filter(job => button.id === "rerun" || job.status === "failed").map(job => job.selection));
    }
    catch (error) {
        showError(error);
...
document.addEventListener("change", event => {
    ...
    if (input.dataset.fileCheck) {
        if (input.checked)
            state.selected.set(input.dataset.fileCheck, null);
        else
            state.selected.delete(input.dataset.fileCheck);
        renderFiles();
        if (!state.runId)
            renderInspector();
    }
    if (input.dataset.caseCheck) {
        ...
        cases ??= new Set();
        if (input.checked)
            cases.add(input.dataset.caseCheck);
        else
            cases.delete(input.dataset.caseCheck);
        if (cases.size)
            state.selected.set(file.id, cases);
        else
            state.selected.delete(file.id);
        renderFiles();
    }
});
$("#search").addEventListener("input", event => { state.query = event.target.value.trim().toLowerCase(); renderFiles(); });
$("#platform").addEventListener("change", event => { state.platform = event.target.value; renderFiles(); });
$("#workspace-picker").addEventListener("change", event => { state.workspace = event.target.value; summary(); renderFiles(); });
