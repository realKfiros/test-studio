...
afterEach(async () => { for (const dispose of cleanup.splice(0).reverse())
    await dispose(); });
...
describe("automatic discovery", () => {
    test("reads nested and aliased declarations without interpreting comments or strings", () => {
        const result = discoverScript(`import { describe as suite, it as check } from 'bun:test';
      // test('fake', () => {});
      ...
      ['one'].forEach(value => check('callback generated', () => {}));`, "a.test.ts");
        expect(result.runner).toBe("bun");
        expect(result.cases?.map(item => [item.fullName, item.runnable])).toEqual([
            ["outer inner a [literal].*", true], ["not now", false], ["value %i", false], ["generated", false], ["callback generated", false],
        ]);
    });
    test("inherits skipped suites and recognizes namespace imports", () => {
        const result = discoverScript(`import * as b from 'bun:test'; b.describe.skip('later', () => { b.test('example', () => {}); }); b.test.todo('soon');`, "b.test.ts");
    ...
    test("requires file-level runs for duplicate names and invalidates renamed test IDs", () => {
        const source = "import {test} from 'bun:test'; test('same', () => {}); test('same', () => {});";
        expect(discoverScript(source, "same.test.ts").cases?.map(item => item.runnable)).toEqual([false, false]);
        expect(discoverScript(source, "same.test.ts").cases?.[0].id).not.toBe(discoverScript(source.replaceAll("same", "changed"), "same.test.ts").cases?.[0].id);
    });
    test("parses Maestro documents and source lines while ignoring ordinary YAML", () => {
        const result = discoverMaestro('appId: host.exp.Exponent\nname: Sign in\ntags: [smoke]\n---\n- launchApp\n- tapOn:\n    id: login\n', "flow-ios.yaml");
        expect(result).toMatchObject({ runner: "maestro", name: "Sign in", platform: "ios", tags: ["smoke"] });
    ...
    test("finds workspace tests, hidden Maestro folders, additions and deletions; excludes dependencies and nested repos", async () => {
        ...
        expect(catalog.files.map(file => file.path)).toEqual([".maestro/login.yaml", "packages/api/one.test.ts"]);
        expect(catalog.files[1].workspace).toBe("packages/api");
        await writeFile(join(root, "new.test.ts"), "import {test} from 'bun:test'; test('new', () => {});");
        await rm(join(root, "packages/api/one.test.ts"));
        catalog = await scanProject(root);
        expect(catalog.files.map(file => file.path)).toEqual([".maestro/login.yaml", "new.test.ts"]);
    ...
    test("respects gitignore while discovering untracked source tests", async () => {
        const root = await fixture({ ".gitignore": "lib/\n", "src/new.test.ts": "import {test} from 'bun:test'; test('new', () => {});", "lib/new.test.js": "import {test} from 'bun:test'; test('generated', () => {});" });
        expect(spawnSync("git", ["init", "--quiet", root]).status).toBe(0);
        expect((await scanProject(root)).files.map(file => file.path)).toEqual(["src/new.test.ts"]);
    });
});
describe("execution adapters", () => {
    test("builds literal Maestro arguments with device/env support and redacted command preview", () => {
        const file = { id: "flow.yaml", path: "flow with spaces.yaml", runner: "maestro", cwd: ".", workspace: ".", name: "flow", cases: [] };
        const command = adapters[1].command("/tmp/project", file, { fileId: file.id }, "/tmp/report.xml", { device: "device-1", env: { QUERY: "$(touch /tmp/no); private" } });
        ...
        expect(() => adapters[1].command("/tmp/project", file, { fileId: file.id, caseIds: ["step"] }, "report", {})).toThrow();
    });
    test("parses nested Bun and flat Maestro JUnit reports including failed and skipped cases", () => {
        const results = parseReport('<testsuites><testsuite><testsuite><testcase name="ok &amp; fine" time="0.1"/><testcase name="bad"><failure message="assertion">details</failure></testcase><testcase name="later"><skipped/></testcase></testsuite></testsuite></testsuites>');
        ...
        expect(parseReport('<testsuite><testcase name="Flow" time="1.2"/></testsuite>')[0].duration).toBe(1200);
    ...
    test("runs a selected real Bun test with regex punctuation and excludes its failing sibling", async () => {
        const root = await fixture({ "nested/example.test.ts": "import {describe,test,expect} from 'bun:test'; describe('suite', () => { test('literal [x].*', () => expect(1).toBe(1)); test('sibling', () => expect(1).toBe(2)); });" });
        ...
        expect(run.jobs[0].results.find(result => result.status === "passed")?.name).toContain("literal [x].*");
    ...
    test("rejects invalid IDs before executing any job and validates request shape", async () => {
        const root = await fixture({ "a.test.ts": "import {test} from 'bun:test'; test('a', () => {});" });
        const runner = new TestRunner();
        await expect(runner.start(await scanProject(root), [{ fileId: "a.test.ts" }, { fileId: "../../other.ts" }])).rejects.toThrow();
        expect(runner.runs).toHaveLength(0);
        expect(() => validateRequest({ selections: [] })).toThrow();
        expect(() => validateRequest({ selections: [{ fileId: "a" }], options: { env: { "BAD KEY": "x" } } })).toThrow();
    });
    test("cancels an active process, cancels queued jobs, and prevents concurrent runs", async () => {
        const root = await fixture({ "a.test.ts": "import {test} from 'bun:test'; test('wait', async () => { console.log('STARTED'); await Bun.sleep(30000); }, 40000);", "b.test.ts": "import {test} from 'bun:test'; test('queued', () => {});" });
        const runner = new TestRunner();
        cleanup.push(() => runner.shutdown());
        const catalog = await scanProject(root);
        const run = await runner.start(catalog, catalog.files.map(file => ({ fileId: file.id })));
        await expect(runner.start(catalog, [{ fileId: "b.test.ts" }])).rejects.toThrow("already active");
    ...
    test("does not report success when a selected name no longer matches", async () => {
        const root = await fixture({ "a.test.ts": "import {test} from 'bun:test'; test('old name', () => {});" });
        const catalog = await scanProject(root);
        await writeFile(join(root, "a.test.ts"), "import {test} from 'bun:test'; test('new name', () => {});");
        const runner = new TestRunner();
        const run = await runner.start(catalog, [{ fileId: "a.test.ts", caseIds: [catalog.files[0].cases[0].id] }]);
        await finished(run);
        expect(run.status).toBe("failed");
...
describe("local API boundary", () => {
    test("requires the session token, rejects foreign origins and limits source access to discovered tests", async () => {
        const root = await fixture({ "a.test.ts": "import {test} from 'bun:test'; test('a', () => {});", "private.txt": "not test source" });
        ...
        expect((await fetch(`${base}/api/catalog`, { headers: { ...headers, Origin: "https://example.com" } })).status).toBe(403);
        ...
        const response = await fetch(`${base}/api/run`, { method: "POST", headers: { ...headers, "Content-Type": "application/json" }, body: JSON.stringify({ selections: [{ fileId: "a.test.ts" }] }) });
