...
export async function startServer(root: string, port = 4310) {
    ...
    const html = (await readFile(new URL("./web/index.html", import.meta.url), "utf8")).replace("__SESSION_TOKEN__", token);
    const refresh = () => scanning ??= scanProject(root).then(value => { catalog = value; }).catch(error => { catalog.warnings = [error.message]; }).finally(() => { scanning = undefined; });
    const json = (value: unknown, status = 200) => Response.json(value, { status, headers: { "Cache-Control": "no-store" } });
    const server = Bun.serve({
        hostname: "127.0.0.1", port, idleTimeout: 30, maxRequestBodySize: 512000,
        async fetch(request) {
            ...
            if (url.pathname.startsWith("/api/") && request.headers.get("x-test-studio-token") !== token)
                return json({ error: "Invalid session" }, 403);
            try {
                if (request.method === "GET" && url.pathname === "/")
                    return new Response(html, { headers: { "Content-Type": "text/html; charset=utf-8", "Cache-Control": "no-store", "Content-Security-Policy": "default-src 'self'; script-src 'self'; style-src 'self'; connect-src 'self'; img-src 'self' data:; frame-ancestors 'none'; base-uri 'none'; form-action 'none'", "X-Content-Type-Options": "nosniff" } });
                ...
                if (request.method === "GET" && url.pathname === "/api/runs")
                    return json(runner.runs.map(({ jobs, ...run }) => ({ ...run, jobs: jobs.map(({ output, results, file, ...job }) => ({ ...job, file: { id: file.id, name: file.name, runner: file.runner, path: file.path }, resultCounts: { passed: results.filter(r => r.status === "passed").length, failed: results.filter(r => r.status === "failed").length, skipped: results.filter(r => r.status === "skipped").length } })) })));
    ...
    return { server, runner, stop: () => { clearInterval(interval); runner.shutdown(); server.stop(true); } };
}
if (import.meta.main) {
    ...
    for (const signal of ["SIGINT", "SIGTERM"] as const)
        process.on(signal, () => { app.stop(); setTimeout(() => process.exit(0), 1700); });
}
